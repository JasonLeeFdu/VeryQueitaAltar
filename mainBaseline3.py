...
def originalVSFAMain():
    ...
    if os.path.exists(save_result_file):
        ...
        SROCC= lastResDict['SROCC'];  KROCC=lastResDict['KROCC']; PLCC=lastResDict['PLCC']; RMSE=lastResDict['RMSE'];
        best_val_criterion = nt.bestCriterion(SROCC, KROCC, PLCC, RMSE)
    ...
    for epoch in range(Epoch + 1, conf.MAX_Epoch):
        # Train for 1 epoch
        if conf.verbose == 1:
        ...
        for i, (motionFeat, distortFeat, contentFeat, label, vidLen) in enumerate(train_loader):
            ii = i
            s = time.time()
            # y_val1[i] = scale * label.numpy()  #
            motionFeat = motionFeat.cuda().float()
            ...
            outputs = model(motionFeat, vidLen, contentFeat, distortFeat)
        ...
        L = 0
        with torch.no_grad():
            for i, (motionFeat, distortFeat, contentFeat, label, vidLen) in enumerate(val_loader):
                y_val[i] = scale * label.to('cpu').numpy()  #
                motionFeat = motionFeat.to(device).float()
                distortFeat = distortFeat.to(device).float()
                contentFeat = contentFeat.to(device).float()
                label = label.to(device).float()
                vidLen = vidLen.to(device).float()
        ...
        if conf.TEST_RATIO > 0 and not conf.NO_TEST_DURING_TRAINING:
            ...
            L = 0
            with torch.no_grad():
                for i, (motionFeat, distortFeat, contentFeat, label, vidLen) in enumerate(test_loader):
        ...
        if nt.bestCriterion(SROCC,KROCC,PLCC,RMSE) > best_val_criterion and epoch > conf.MAX_Epoch / 7:  ##$$##
            ...
            best_val_criterion = nt.bestCriterion(SROCC,KROCC,PLCC,RMSE)  # update best val SROCC     ##$$##
